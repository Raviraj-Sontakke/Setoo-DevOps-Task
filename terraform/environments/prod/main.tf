...
module "networking" {
  ...
  vnet_address_space     = ["10.2.0.0/8"]
  aks_subnet_prefixes    = ["10.242.0.0/16"]
  app_gw_subnet_prefixes = ["10.243.0.0/24"]
  tags                   = local.tags
}
...
module "storage" {
  ...
  storage_account_name = "${local.prefix}storageprod"
  resource_group_name  = module.networking.resource_group_name
  location             = var.location
  tags                 = local.tags
}
